...
export async function GET(request: NextRequest) {
  try {
    ...
    const url = new URL(`${WORDPRESS_API_BASE}/magazines_cat`);
    ...
    if (!response.ok) {
      return NextResponse.json(
        { error: "Failed to fetch magazines categories from WordPress." },
    ...
    return NextResponse.json(
      { error: "Unexpected error while loading magazines categories." },
      { status: 500 },
    );
  }
}
