...
export async function GET(request: NextRequest) {
  try {
    ...
    const url = new URL(`${WORDPRESS_API_BASE}/knesset_cat`);
    ...
    if (!response.ok) {
      return NextResponse.json(
        { error: "Failed to fetch knesset categories from WordPress." },
    ...
    return NextResponse.json(
      { error: "Unexpected error while loading knesset categories." },
      { status: 500 },
    );
  }
}
